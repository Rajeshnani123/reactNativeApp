...
export const verifyOtp = (data,Otp) => {
    return async(dispatch) => {
        ...
        try{
            ...
            if(response && response.message){
                dispatch({type: USER_VERIFY_OTP_SUCCESS, verifyMessage: response.message});
            }else{
...
export const resetPassword = (Body) => {
    return async(dispatch) => {
        dispatch({type: USER_RESET_LOADING});
        try{
            const response = await put(ApiConnections.resetPassword,Body);
            if(response && response.message){
                dispatch({type: USER_RESET_SUCCESS, resetMessage: response.message});
            }else{
                dispatch({type: USER_RESET_FAILURE, error: "Data not found"});
            }
        }catch(error){
            dispatch({type: USER_RESET_FAILURE,error: error})
        }
    }
}